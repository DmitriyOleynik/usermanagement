...
public class UserTest {
    ...
    //Когда день рождения был ранее в этом месяце
    @Test
    public void testAgeTheSameMonthAfter() {
        Calendar calendar = Calendar.getInstance();
        calendar.set(YEAR_OF_BIRTH, Calendar.OCTOBER, 1);
        User user = new User(ID, FIRST_NAME, LAST_NAME, calendar.getTime());
        assertEquals(28, user.getAge());
    }

    //Когда день рождения будет позже в этом месяце
    @Test
    public void testAgeTheSameMonthBefore() {
        Calendar calendar = Calendar.getInstance();
        calendar.set(YEAR_OF_BIRTH, Calendar.NOVEMBER, 1);
        User user = new User(ID, FIRST_NAME, LAST_NAME, calendar.getTime());
        assertEquals(27, user.getAge());
    }
